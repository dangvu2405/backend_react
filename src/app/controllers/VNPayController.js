...
class VNPayController {
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'production') {
                ...
                if (hasLocalhost || notHttps) {
                    console.warn('⚠️ WARNING: VNPAY_RETURN_URL có vấn đề trong production:', {
                ...
                if (ipnHasLocalhost || ipnNotHttps) {
                    console.warn('⚠️ WARNING: VNPAY_IPN_URL có vấn đề trong production:', {
    ...
    async createQRCode(req, res) {
        try {
            ...
            vnp_ReturnUrl = vnp_ReturnUrl.trim().replace(/\s+/g, '').replace(/\/+/g, '/').replace(/:\/([^/])/, '://$1');
            if (vnp_ReturnUrl.includes('localhost') || vnp_ReturnUrl.includes('127.0.0.1')) {
                console.warn('⚠️ WARNING: vnp_ReturnUrl có localhost (QR), thay thế bằng production URL');
                vnp_ReturnUrl = 'https://dtv2405.id.vn/payment/vnpay-return';
            }
            ...
            vnp_IpnUrl = vnp_IpnUrl.trim().replace(/\s+/g, '').replace(/\/+/g, '/').replace(/:\/([^/])/, '://$1');
            if (vnp_IpnUrl.includes('localhost') || vnp_IpnUrl.includes('127.0.0.1')) {
                console.warn('⚠️ WARNING: vnp_IpnUrl có localhost (QR), thay thế bằng production URL');
                vnp_IpnUrl = 'https://backend-ks46.onrender.com/api/payment/vnpay/ipn';
            }
